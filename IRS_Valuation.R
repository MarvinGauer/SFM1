...
Maturity = c(0,0.5,1,1.5,2,2.5) # Maturities corresponding to the yields
Yields = c(0,2*(1/0.97)-2,(1/0.94)-1,((1/0.91)-1)/3*2,((1/0.87)-1)/2,0.08) # Yields corresponding to the maturities
...
# Simple compounding Forward Rates implied by the Yield Curve
...
VSwapFRA = sum(unlist(lapply(1:(nrow(y)-1),function(x) FRA(Rfix,y,y[x,1],y[x+1,1],P)))) # Sumation of different FRAs

######## 2.4. Valuation in Terms of discounted Cash Flows

DCF = P*c(0,as.numeric((sapply(2:(nrow(y)), function(x) (y[x,2]*y[x,1]+1)^(-1)) * diff(y[,1])) * (Rfix - sapply(1:(nrow(y)-1),function(x) ForwardRates(y,y[x,1],y[x+1,1])))))
VSwapR = sum(DCF)
